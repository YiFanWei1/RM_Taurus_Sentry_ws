...
: Node("lifecycle_manager", options), diagnostics_updater_(this)
{
  ...
  registerRclPreshutdownCallback();//在该回调函数中，可以执行一些清理操作或释放资源的操作。
  ...
  //create_service()函数被用来创建两个服务，即manager_srv_和is_active_srv_
  //创建服务并将它们绑定到回调组callback_group_上，以便在节点运行时处理服务请求。
...
LifecycleManager::startup()
{
  ...
  if (!changeStateForAllNodes(Transition::TRANSITION_CONFIGURE) ||//将node组执行configure和active
