...
{
  wait_end_ = node_.lock()->now() + rclcpp::Duration(0.5,0);
  std::cout << "rclcpp::Duration(command->time): " << rclcpp::Duration(command->time).seconds() <<std::endl;
  RCLCPP_ERROR(logger_, "throw nav2_core::PlannerException(Invalid path, Path is empty.);!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
