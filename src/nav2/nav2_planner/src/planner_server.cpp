...
PlannerServer::on_configure(const rclcpp_lifecycle::State & /*state*/)
{
  ...
  costmap_ros_->configure();//测试或验证生命周期节点配置阶段的函数
  costmap_ = costmap_ros_->getCostmap();//这个成本图将用于后续的路径规划计算。

  // Launch a thread to run the costmap node
  costmap_thread_ = std::make_unique<nav2_util::NodeThread>(costmap_ros_);//用于在单独的线程中运行成本图节点。这有助于提高系统的整体性能，因为成本图的处理可以并行进行。

  RCLCPP_DEBUG(
    get_logger(), "Costmap size: %d,%d",
    costmap_->getSizeInCellsX(), costmap_->getSizeInCellsY());//打印成本图的尺寸（以网格单元的数量表示）

  tf_ = costmap_ros_->getTfBuffer();//从成本图 ROS 包装器中获取变换（Transform）缓存。变换缓存用于处理不同坐标系之间的位置和方向转换。

  planner_types_.resize(planner_ids_.size());//根据路径规划器标识符的数量调整 planner_types_ 数组的大小。这为每个路径规划器准备存储其类型名称的空间。
