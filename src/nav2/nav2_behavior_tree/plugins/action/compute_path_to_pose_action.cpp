  ...
  BT::NodeBuilder builder =
    [](const std::string & name, const BT::NodeConfiguration & config)
    {
      return std::make_unique<nav2_behavior_tree::ComputePathToPoseAction>(
        name, "compute_path_to_pose", config);//这里和planner有关，一直发client的请求让planner里面的action_server_pose_服务端进入回调
    };

  factory.registerBuilder<nav2_behavior_tree::ComputePathToPoseAction>(
    "ComputePathToPose", builder);//这里就是行为树和代码的关系,定义了ComputePathToPose名字的孩子，然后创建了compute_path_to_pose的客户端进行请求
}
